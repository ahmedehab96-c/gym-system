...
export function Sidebar({ mobileOpen, onCloseMobile }: SidebarProps) {
  ...
  const visibleSections = navSections
    .map((section) => ({ ...section, items: section.items.filter((item) => !item.module || hasPermission(item.module, "view")) }))
      ...
      >
        <div className="flex items-center justify-between px-5 py-5">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-a-accent to-a-accent-2 text-black">
              <Dumbbell size={18} strokeWidth={2.5} />
            </div>
            <div>
              <p className="text-sm font-bold leading-none text-a-text dark:text-a-dark-text">Premium Gym</p>
              <p className="mt-0.5 text-[11px] text-a-muted dark:text-a-dark-muted">Admin Console</p>
            </div>
          </div>
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      cn(
                        ...
                        isActive
                          ? "bg-a-accent/10 text-a-accent-2 dark:text-a-accent"
